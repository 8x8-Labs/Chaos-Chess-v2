...
Cards are the primary mechanism for rewriting chess rules mid-match. Full contract — `CardDataSO` config, `ICard`/`CardData` behaviour classes, and the `PieceEffector`/`TileEffector`/`GlobalEffector` lifecycle — is in `Assets/Script/Card/CLAUDE.md`, which loads when working under that directory. Also read `Card/Effector/Effector.md` before writing or modifying any effect.
