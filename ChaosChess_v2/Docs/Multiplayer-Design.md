...
| 단계 | 내용 | 이 순서인 이유 |
|---|---|---|
| **0** | `GameMode.Multiplayer` 추가 + **`LocalColor` 도입(흑 플레이 가능화)** + 턴 프로바이더 인터페이스 추상화 | 네트워크 없이 "핫시트 2인"으로 검증 가능. 여기서 버그를 다 잡고 가야 함. **이걸 안 하면 나중에 전부 다시 손대야 한다** |
| **1** | `IMatchTransport` 추상화 + 로컬 루프백 구현 | 에디터 1개에서 양쪽 시뮬레이션 |
| **2** | UGS Auth(GPGS 연동) + Lobby/Relay 실제 2인 연결 | 여기서 처음 진짜 통신 |
...
**0단계(`LocalColor` 도입 + 턴 프로바이더 추상화)부터 시작할 것을 권장한다.**
네트워크 코드를 한 줄도 쓰지 않고 멀티 준비도의 절반이 끝나며,
이 리팩터를 건너뛰면 이후 모든 단계가 재작업 대상이 된다.
