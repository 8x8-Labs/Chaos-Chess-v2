...
4. **로컬 쓰기는 원자적입니다** (`Assets/Script/Save/SafeFile.cs`). `File.WriteAllText`는 원자적이지 않아, 모바일에서 쓰기 도중 OS가 앱을 정리하면 잘린 JSON이 남습니다. 임시 파일에 먼저 쓰고 rename으로 교체하며 직전 세대를 `.bak`으로 한 번 보존합니다. 읽기는 `본 파일 → .tmp → .bak` 순으로 폴백합니다.

> **주의**: 이건 손상 방지이지 **보안이 아닙니다.** 파일은 평문이고 사용자가 편집할 수 있습니다. Easy Save 같은 에셋의 AES 암호화도 비밀번호가 빌드에 박혀 있어 캐주얼한 변조를 귀찮게 만드는 수준입니다. 실제 변조 방지는 서버 권위 검증으로만 가능합니다.
